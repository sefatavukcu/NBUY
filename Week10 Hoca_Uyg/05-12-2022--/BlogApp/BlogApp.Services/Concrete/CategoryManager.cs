...
namespace BlogApp.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        {
            var result = await _unitOfWork.Categories.AnyAsync(c => c.Id == categoryId); // Efentityrepobaseye gider ve oradan veritabanı bağlantısını sorgular
            if (result)
            {
                var category = await _unitOfWork.Categories.GetAsync(c => c.Id == categoryId); //efentityrepobaseye giderek getasync metodunu çalıştırır. unitofwork yardımıyla

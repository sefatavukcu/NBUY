    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Manage(string id)
        {
            ...
            genderList.Add(new SelectListItem
            {
                Text = "Erkek",
                Value = "Erkek",
                Selected = user.Gender == "Erkek" ? true : false, // hesabı yönet kısmında cinsiyeti görmek için koşul yaptık.
            });
        ...
        public async Task<IActionResult> Manage(UserManageDto userManageDto)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = await _userManager.FindByEmailAsync(userManageDto.Email);
                ...
                if (result.Succeeded)
                {
                    TempData["Message"] = Jobs.CreateMessage("Başarılı", "Bilgileriniz başarıyla güncellendi.", "success");
                }
                List<SelectListItem> genderList = new List<SelectListItem>();
                genderList.Add(new SelectListItem
                {
            ...
            ModelState.AddModelError("", "Bilgilerini güncellenemedi. Tekrar deneyiniz.");
            return View(userManageDto);
        }
    }
}
